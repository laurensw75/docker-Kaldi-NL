...
class ServerWebsocket(WebSocketClient):
    ...
    def __init__(self, uri, description, decoder_pipeline, post_processor, full_post_processor=None):
        self.uri = uri
    ...
    def opened(self):
        ...
        self.last_partial_result = ""
        event = dict(description=self.description)
        try:
	         self.send(json.dumps(event))
        except:
            logger.warning("%s: Failed to send event to master" % (self.request_id))
...
def main_loop(uri, description, decoder_pipeline, post_processor, full_post_processor=None):
    while True:
        ws = ServerWebsocket(uri, description, decoder_pipeline, post_processor, full_post_processor=full_post_processor)
        try:
...
def main():
    ...
    main_loop(args.uri, description, decoder_pipeline, post_processor, full_post_processor)  
